...
export default function MassMessagingPage() {
  ...
  const sendSimpleMassMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Não limpar o formulário para facilitar reenvios
      // simpleMessageForm.reset();
    },
    onError: (error: Error) => {
      toast({
  ...
  const sendFlowMassMutation = useMutation({
    mutationFn: async (data: { instanceId: string, flowId: string, contacts: string[], delayBetweenContacts: number }) => {
      // Esta API é fictícia, assumimos que o backend tem suporte para isso
  ...
  // Mensagens em progresso? (apenas durante o envio efetivo)
