...
import { Instance, MessageFlow, messageTypeEnum } from "@shared/schema";
...
interface EditMessageFlowDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  selectedFlow?: MessageFlow;
}

export function EditMessageFlowDialog({ 
  open, 
  onOpenChange, 
  selectedFlow 
}: EditMessageFlowDialogProps) {
  ...
  // Quando o modal abre ou quando o fluxo selecionado muda
  useEffect(() => {
    if (open && selectedFlow) {
      // Processa as mensagens
      let messages = [];
      try {
        if (typeof selectedFlow.messages === 'string') {
          messages = JSON.parse(selectedFlow.messages);
        } else if (Array.isArray(selectedFlow.messages)) {
          messages = selectedFlow.messages;
        }
        
        // Assegura que temos um array válido
        if (!Array.isArray(messages)) {
          messages = [{ type: "text", text: "", delay: 3 }];
        }
        
        // Para mensagens antigas que não possuem o campo 'type', adiciona como 'text'
      ...
      // Preenche o formulário com os dados do fluxo
      form.reset({
        name: selectedFlow.name,
        keyword: selectedFlow.keyword,
        instanceId: selectedFlow.instanceId,
        messages,
        triggerType: selectedFlow.triggerType || "exact_match",
        activationDelay: selectedFlow.activationDelay || 0
      });
    }
  }, [open, selectedFlow, form]);

  const updateMessageFlowMutation = useMutation({
    mutationFn: async (values: MessageFlowFormValues) => {
      if (!selectedFlow) throw new Error("Nenhum fluxo selecionado para edição");
      
      const res = await apiRequest("PUT", `/api/message-flows/${selectedFlow.id}`, values);
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/message-flows"] });
      toast({
        title: "Fluxo de mensagens atualizado",
    ...
    onError: (error: Error) => {
      toast({
        title: "Erro ao atualizar fluxo de mensagens",
  ...
  function onSubmit(values: MessageFlowFormValues) {
    updateMessageFlowMutation.mutate(values);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar Fluxo de Mensagens</DialogTitle>
          <DialogDescription>
            Modifique as configurações do seu fluxo de mensagens automáticas.
            Reorganize as mensagens arrastando-as na sequência desejada.
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={updateMessageFlowMutation.isPending}>
                {updateMessageFlowMutation.isPending && (
